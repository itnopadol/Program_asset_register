...
<div class="container">
	...
	<!-- Static navbar -->
		...
		<a class="navbar-brand"  id="sizezi">รายการเบิกการวัสดุ / อุปกรณ์</a>
...
<?php
	...
	$result2 = mysqli_query($con,"SELECT * FROM spare_part WHERE  name  LIKE '%$keyword%' OR category LIKE '%$keyword%'OR brand  LIKE '%$keyword%' OR category =(SELECT Category_id FROM category_spare WHERE Category_name  LIKE '%$keyword%' LIMIT 1) ORDER BY time DESC LIMIT $start_rows,$rowspage")or die("SQL Error2".mysqli_error($con));
...
?>
	...
	<table border='0' align='center' width='94%' >
	<tr><td>
	...
	echo "<td align='center'  style='padding-top:2%;cursor:pointer;'><img src='../img/if_pencil_10550.png'  width='35'  height='35' onClick=\"Omdz('$id', '$name', '$brand', '$price', '$category', '$stock', '$Pay', '$balance', '$acquire')\"></TD>";
	
	echo "<td align='center'  style='padding-top:2%' ><a href='delete_spare.php?id=$id'><img src='../img/cancel.png'  width='35'  height='35'></TD>";
	echo "</tr>"; 
...
<script type="text/javascript">
	function Omdz(id, name, brand, price, category, stock, balance, acquire,time){
		 $("#idz").modal('show');
		 
		  document.getElementById("NewID").value = id;
		  document.getElementById("namez").value = name;
		  document.getElementById("brandz").value = brand;
		  document.getElementById("categoryz").value = category;
		  document.getElementById("pricez").value = price;
		  document.getElementById("stockz").value = stock; 
		  document.getElementById("timez").value = time;
	
	}
</script>
<p align="center"><a href="menu_rent.php">กลับหน้า Index</a> || <a href="add_spare.php">เพิ่มรายการ</a> || <a href="Untitled-9.php">สั่งพิม</p></a>

<div class="modal fade" id="idz" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content" style="margin-top: 170px;">
      <div class="modal-header">
      ...
      <div class="modal-body">
          <div class="form-group">          
           ...
           <label for="recipient-name" class="control-label">รหัสวัสดุ :</label>
           <input type="text" class="form-control" name="NewID " id="NewID"  readonly style="font-size:22px"></label>
                      <label for="recipient-name" class="control-label">รายการ :</label>
            <input type="text" class="form-control" name="namez" id="namez"  style="font-size:22px"></label>
            
           <label for="recipient-name" class="control-label">รูปภาพ:</label>
           <input type="file" class="form-control"  name="photo" id="photo" style="font-size:15px"></label>
            
 
            
            <label for="recipient-name" class="control-label">รุ่น/ยี่ห้อ :
            <input type="text" class="form-control" name="brandz" id="brandz" style="font-size:20px"></label>
            
            <label for="recipient-name" class="control-label">ประเภท :
            <select class="form-control" name="categoryz" id="categoryz"  style="font-size:20px">
             
             </select><p>
             </label>
            <label for="recipient-name" class="control-label">ราคา :
            <input type="text" class="form-control" id="pricez" name="pricez"   style="font-size:20px"></label>
            
            <label for="recipient-name" class="control-label">จำนวน Stock :
            <input type="text" class="form-control" id="stockz" name="stockz"  style="font-size:20px"></label><p>
            
             <label for="recipient-name" class="control-label">วันที่แก้ไข :</label>
            <input type="date" class="form-control" name="timez" readonly  value="<?php echo date("Y-m-d") ?>"  style="font-size:20px"> </label>
      ...
      <div class="modal-footer">
      <input type="submit" value="ตกลง" class="btn btn-success"  style="font-size:20px">
      <button type="reset" class="btn btn-danger" data-dismiss="modal" style="font-size:20px">ยกเลิก</button>  
        </form>
