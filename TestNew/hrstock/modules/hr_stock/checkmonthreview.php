...
<table cellpadding="0" cellspacing="0" border="1s">
	<tr>
		...
		<td bgcolor="#F9F9F9" colspan="<?=count($sdate);?>">รายงานยอดเบิกแต่ละวัน</td>
	...
	<?php
		...
		while($row=mysql_fetch_array($rs)){
			...
			for($i=0;$i<count($sdate);$i++){
				...
				$sql="select sum(beg) as begs From stock_tb_beg_master_sub INNER JOIN stock_tb_beg_master ON stock_tb_beg_master_sub.nobeg=stock_tb_beg_master.nobeg where stock_tb_beg_master_sub.kindtypeid='".$row['kindtypeid']."' and stock_tb_beg_master.datebegtotal='".date("Y-m-d",strtotime($sdate[$i]))."' And stock_tb_beg_master.statusbeg='1' And stock_tb_beg_master_sub.receive='on'";
