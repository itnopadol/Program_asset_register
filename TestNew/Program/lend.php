...
include("../function/db_function.php");//include ไฟล์ที่เขียนฟังก์ชั่นไว้ใช้งาน
	...
	$result = mysqli_query($con, "SELECT * FROM lend_spare WHERE id_spare LIKE '%$keyword%' OR  id_spare LIKE '%$keyword%'OR detail ORDER BY id_spare ASC") or die ("MySQL =>".mysqli_error($con));	
...
มีทั้งหมด $rows รายการ </p>";
	...
	while(list($id_spare,$name,$detail,$category_lend,$amount,$Order_lend,$lend_data,$rent_empID) = mysqli_fetch_row($result)){ 
