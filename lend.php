...
<div class="container">
	...
	<!-- Static navbar -->
		...
		<nav class="navbar navbar-expand-lg navbar-light bg-light">
			<img src='../img/678126-box-in-128.png'  width='60'  height='60'><a class="navbar-brand" href="#" id="sizezi">ประวัติรายการเบิกการวัสดุ / อุปกรณ์</a>
...
include("../function/db_function.php");//include ไฟล์ที่เขียนฟังก์ชั่นไว้ใช้งาน
	...
	$result = mysqli_query($con,"SELECT lend_spare.*,lend_empsp.rent_name FROM lend_spare Left JOIN lend_empsp ON lend_spare.rent_empID=lend_empsp.rent_empID  WHERE lend_empsp.rent_name
	
 LIKE '%$keyword%' OR Order_lend LIKE '%$keyword%' OR detail  LIKE '%$keyword%' OR category_lend =(SELECT Category_id FROM category_spare WHERE Category_name  LIKE '%$keyword%' ORDER BY lend_spare.No ASC)")or die("SQL Error2".mysqli_error($con));
	
	$row=mysqli_num_rows($result); //จำนวนแถวที่คิวรี่ออกมาได้
	$rowspage=15;//กำหนดจำนวนแถวที่จะแสดงในแต่ละหน้า
	$page=ceil($row/$rowspage);
	 ...
	 $start_rows=($page_id*$rowspage)-$rowspage; //แถวแรกที่จะแสดงในแต่ละหน้า(หมายเลขหน้า * จำนวนแถวใน 1 หน้า) จำนวนแถวใน 1 หน้า
	  ...
	  $result2= mysqli_query($con," SELECT lend_spare.*,lend_empsp.rent_name FROM lend_spare Left JOIN lend_empsp ON lend_spare.rent_empID=lend_empsp.rent_empID  WHERE  lend_spare.rent_empID 
	  
LIKE '%$keyword%' OR Order_lend LIKE '%$keyword%' OR detail  LIKE '%$keyword%' OR category_lend =(SELECT Category_id FROM category_spare WHERE Category_name  LIKE '%$keyword%' ORDER BY lend_spare.No ASC LIMIT $start_rows,$rowspage)")or die("SQL Error2".mysqli_error($con));
